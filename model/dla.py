...
class DLASeg(BaseModel):
    def __init__(self, base_name, heads, pretrained=False, down_ratio=4, head_conv=256):
        ...
        for head in self.heads:
            ...
            if head_conv > 0:
                fc = nn.Sequential(
                    nn.ConvTranspose2d(channels[self.first_level], head_conv,
                                       kernel_size=2, stride=2, bias=True),
                    nn.ReLU(inplace=True),
                    nn.Conv2d(head_conv, head_conv,
                              kernel_size=3, padding=1, bias=True),
                    nn.ReLU(inplace=True),
                    nn.ConvTranspose2d(head_conv, classes,
                                       kernel_size=2, stride=2, bias=True))
                fill_fc_weights(fc)
            else:
    ...
    def forward(self, x):
        ...
        if len(self.heads) == 1:
            ret = self.__getattr__(list(self.heads.keys())[0])(x)
        else:
            ret = []
            for head in self.heads:
